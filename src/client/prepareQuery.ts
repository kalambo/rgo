...
const getFields = (obj: any): string[] => {
  ...
  if (isObject(obj)) {
    return Object.keys(obj).reduce((res, k) => [...res,
      ...(k[0] === '$' ? getFields(obj[k]) : [k])
    ], [] as string[]);
  }
  ...
  const apiQuery = visit(parse(query), {
    ...
    Field(node: FieldNode) {
      ...
      if (sels) {
        ...
        const fields = Array.from(new Set(newFields)).filter(f =>
          !sels.some(s => s.kind === 'Field' && s.name.value === f)
        );
        ...
        if (fields.length > 0) {
          return {
            ...
            selectionSet: {
              ...
              selections: [
                ...sels,
                ...fields.map(f => ({ kind: 'Field', name: { kind: 'Name', value: f } })),
              ],
            },
          } as FieldNode;
        }
